...
export async function bootstrapToolServer(cfg: ToolServerConfig): Promise<void> {
  ...
  if (result.isErr()) {
    console.error(`[tool-server] register failed: ${result.error.message}`)
    return
  }

  instanceId = result.value.instanceId
  console.log(`[tool-server] registered as ${cfg.instanceType} (instanceId=${instanceId}, nixPkgs=[${cfg.nixPkgs.join(",")}])`)

  const intervalSec = cfg.heartbeatIntervalSec ?? 30
  heartbeatTimer = setInterval(async () => {
    const hb = await client.heartbeat(instanceId)
    if (hb.isErr()) {
      console.error(`[tool-server] heartbeat failed: ${hb.error.message}`)
    }
  }, intervalSec * 1000)
}
