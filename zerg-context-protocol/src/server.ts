...
import type { IZcpTool, GetContext } from "./tool.js"

export type ToolServiceRouter = (router: ConnectRouter) => void

export interface ZcpServerOptions {
  tools: IZcpTool[]
  getContext: GetContext
  systemContext?: string | (() => string | Promise<string>)
}

export function createZcpServer(options: ZcpServerOptions): ToolServiceRouter
export function createZcpServer(tools: IZcpTool[], getContext: GetContext): ToolServiceRouter
export function createZcpServer(toolsOrOptions: IZcpTool[] | ZcpServerOptions, getContext?: GetContext): ToolServiceRouter {
  const opts: ZcpServerOptions = Array.isArray(toolsOrOptions)
    ? { tools: toolsOrOptions, getContext: getContext! }
    : toolsOrOptions

  const toolMap = new Map<string, IZcpTool>()
